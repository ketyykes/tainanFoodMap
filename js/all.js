...
function renderPopUp() {
	...
	for (let i = 0; i < partOfDistrctAry.length; i++) {
		...
		function makeNotYetSuppyly(number, placeParameterStr) {
			if (
				partOfDistrctAry[number][placeParameterStr] == null ||
				partOfDistrctAry[number][placeParameterStr] == ""
			) {
		...
		markers.addLayer(
			L.marker(position, { icon: greenIcon }).bindPopup(
				`<div class="card border-0">
                    <div class="card-body">
                    <h5 class="card-title"><i class="fas fa-store me-1 text-blue-800"></i>${
											partOfDistrctAry[i].name
										}</h5>
                        <div class="info-responsive">
                            <p class="mt-1">
                            <i class="fas fa-info-circle pe-1 text-blue-800"></i>${
															partOfDistrctAry[i].summary
														}
                            </p>
                        </div>
                        <p class="mt-1">
                        ...
                        ${
													partOfDistrctAry[i].services.length != 0
														? `<p class="mt-1">
                                <i class="fas fa-concierge-bell text-yellow-400 pe-1"></i>${partOfDistrctAry[
																	i
																].services.map((el) => `${el}`)}
                            </p>`
														: ""
                        ...
                        ${
													partOfDistrctAry[i].category.length != 0
														? `<p class="mt-1">
                            <i class="fas fa-tag text-teal-400 pe-1"></i>${partOfDistrctAry[
															i
														].category.map((el) => `${el}`)}
                            </p>`
														: ""
...
function filterAfterAddEvent(elObj, e) {
	if (elObj == "services") {
		partOfDistrctAry = allDataAry.filter((el) => {
			...
			// })
			return (
				el[elObj].includes(e.target.value) &&
				el.lat !== null &&
				el.long !== null
			);
		});
		...
		partOfDistrctAry = allDataAry.filter((el) => {
			return (
				el[elObj] === e.target.value && el.lat !== null && el.long !== null
			);
		});
...
function renderInDataPlace(data, searchType) {
	...
	data.forEach(function (el, idx) {
		if (el.open_time == false) {
			el.open_time = "尚未提供";
		}
		getDataPlace.innerHTML += `
        <li class="rounded mt-2 px-2 py-2 d-flex flex-column align-items-center" data-num=${idx}>
            <h5 class="text-center fs-5 fw-bold text-blue-800 border-bottom border-dark p-1"><i class="fas fa-store me-1"></i>${
							el.name
						}</h5>
...
getDataPlace.addEventListener("click", function (e) {
	...
	let position = [
		partOfDistrctAry[getDataSetNum].lat,
		partOfDistrctAry[getDataSetNum].long,
	];
